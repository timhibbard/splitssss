 ...
 * Who is in a race.
 *
 * The team list is everyone on the phone for the season. A race is run by some of
 * them: seven in the varsity race, the rest in the JV race, and that split moves
 * week to week. The race carries its own lineup so the buttons on the capture
 * screen are the runners actually out on the course.
 */
